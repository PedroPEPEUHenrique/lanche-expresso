...
const options: {
  ...
  { icon: 'person-circle-outline', title: 'Meus dados', subtitle: 'Editar nome, e-mail e telefone', route: '/user/edit-profile' },
  { icon: 'location-outline', title: 'Endereço de entrega', subtitle: 'Alterar meu endereço', route: '/user/edit-address' },
  { icon: 'notifications-outline', title: 'Notificações', subtitle: 'Preferências de alertas' },
  { icon: 'log-out-outline', title: 'Desconectar', subtitle: 'Sair da conta', danger: true },
];
    ...
    <View className="flex-1 bg-white">
      ...
      <View className="mx-5 rounded-2xl overflow-hidden bg-gray-50">
        {options.map((opt, index) => (
          ...
          >
            <View className={`w-10 h-10 rounded-xl items-center justify-center mr-4 ${opt.danger ? 'bg-red-100' : 'bg-sky-100'}`}>
              <Ionicons name={opt.icon} size={22} color={opt.danger ? '#FF6B6B' : '#7EC8E3'} />
            </View>
            <View className="flex-1">
              <Text className={`text-base font-bold ${opt.danger ? 'text-red-400' : 'text-gray-800'}`}>{opt.title}</Text>
              <Text className="text-xs text-gray-400 mt-0.5">{opt.subtitle}</Text>
            </View>
            {!opt.danger && <Ionicons name="chevron-forward" size={18} color="#ccc" />}
          </TouchableOpacity>
        ))}
      </View>
